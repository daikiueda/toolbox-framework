...
const TEMPLATE_WITH_SALESFORCE_DIR = './packages/__template-with-salesforce';
...
const parseArgs = () => {
  ...
  // フラグのパース
  const salesforceIndex = args.indexOf('--salesforce');
  const useSalesforce = salesforceIndex !== -1;
  if (useSalesforce) {
    args.splice(salesforceIndex, 1);
  }

  const electronIndex = args.indexOf('--electron');
  const integrateElectron = electronIndex !== -1;
  ...
  if (args.length === 0) {
    ...
    console.log('💡 使用例:');
    console.log('   npm run create:workspace -- my-feature');
    console.log('   npm run create:workspace -- my-feature --salesforce');
    console.log('   npm run create:workspace -- my-feature --salesforce --electron');
    process.exit(1);
  }
  ...
  return { featureName, useSalesforce, integrateElectron };
};
...
const copyTemplate = (featureName, useSalesforce) => {
  const templateDir = useSalesforce ? TEMPLATE_WITH_SALESFORCE_DIR : TEMPLATE_DIR;
  const targetDir = path.join(PACKAGES_DIR, featureName);
  console.log(`📂 テンプレートを複製中: ${templateDir} → ${targetDir}`);
...
const replaceTemplateIdentifiers = (featureName) => {
  ...
  const replacements = [
    { search: /@toolbox\/template-with-salesforce/g, replace: `@toolbox/${featureName}` },
...
const runInitialSetup = (featureName, useSalesforce) => {
...
const main = () => {
  ...
  const { featureName, useSalesforce, integrateElectron } = parseArgs();
  console.log(`📋 機能名: ${featureName}`);
  console.log(`📋 テンプレート: ${useSalesforce ? 'Salesforce 連携版' : '汎用版'}`);
  if (integrateElectron) {
    console.log(`📋 Electron 連携: 有効`);
  }
  console.log();

  checkExistingDirectory(featureName);
  copyTemplate(featureName, useSalesforce);
  ...
  runInitialSetup(featureName, useSalesforce);
